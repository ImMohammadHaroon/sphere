...
export async function uploadCommentAttachment(taskId, commentId, file) {
...
export async function downloadCommentAttachment(taskId, commentId, attachmentId) {
  const headers = {};
  const accessToken = getAccessToken();
  if (accessToken) {
    headers.Authorization = `Bearer ${accessToken}`;
  ...
  const response = await fetch(
    `${API_URL}/tasks/${taskId}/comments/${commentId}/attachments/${attachmentId}/download`,
    {
      method: "GET",
      headers,
      credentials: "include",
