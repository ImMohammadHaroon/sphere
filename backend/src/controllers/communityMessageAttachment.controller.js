...
import { formatCommunityMessage } from "./communityMessage.controller.js";
...
export function formatCommunityMessageAttachment(attachment) {
...
async function assertCommunityMessageReadable(req, messageId) {
  const message = await req
    .scopedFindOne(CommunityMessage, { _id: messageId })
...
export async function uploadCommunityMessageAttachment(req, res, next) {
  try {
    if (!req.file) {
      throw badRequest("No file uploaded");
    }

    const message = await assertCommunityMessageReadable(
      req,
    ...
    const attachmentPayload = formatCommunityMessageAttachment(attachment);
    ...
    emitToOrg(
      ...
      "community:attachment:new",
      {
        messageId: message._id.toString(),
        attachment: attachmentPayload,
        message: formatCommunityMessage(message, allAttachments),
      }
    );
...
export async function downloadCommunityMessageAttachment(req, res, next) {
  try {
    await assertCommunityMessageReadable(req, req.params.messageId);
...
export async function deleteCommunityMessageAttachment(req, res, next) {
  try {
    const message = await assertCommunityMessageReadable(
      req,
      req.params.messageId
    ...
    emitToOrg(req.user.organizationId, "community:attachment:deleted", {
      messageId: message._id.toString(),
...
export async function listAttachmentsForCommunityMessages(req, messageIds) {
  if (!messageIds.length) {
    return new Map();
  }

  const attachments = await req
    .scopedQuery(Attachment, { communityMessageId: { $in: messageIds } })
    .select(METADATA_SELECT)
    .populate("uploaderId", USER_PUBLIC_FIELDS)
    .sort({ createdAt: 1 })
    .lean();
  ...
  for (const attachment of attachments) {
    const key = attachment.communityMessageId.toString();
    const list = grouped.get(key) ?? [];
    list.push(formatCommunityMessageAttachment(attachment));
